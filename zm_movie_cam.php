...
define('ZMUSER', 'zmuser');
define('ZMPASS', 'zmpass');
define('ZM_DB', 'zm');
define('PATH_EVENT', '/usr/share/zoneminder/events');
...
if (isset($argc)) {
	if ($argc == 8) {
		...
		Make_Movie($MonitorId,$StartTime,$EndTime,$buffer,$Speed,$BitRate,$FrameType);	
		exit;
	}
}
else
...
// POST data from user input
	...
	if (isset($_GET['form']) && $_GET['form'] == 2) {
		...
		{
			Make_Movie($CamId, $Starttime, $Endtime, $Buffer, $Speed, $Bitrate, $FrameType);
		}
		else
		{
...
// check if process (movies) are already being processed and if so, display them
	...
	if ($x > 1) {
		...
		echo $x-1 . ' movie(s) in progress...<br><br>'; }
...
function Load_Camera()
{
	$con=mysqli_connect("localhost",ZMUSER, ZMPASS, ZM_DB);
	...
	// 
	$result = mysqli_query($con,"SELECT Id, Name from Monitors");
...
function Make_Movie($MonitorId,$StartTime,$EndTime,$buffer,$Speed,$BitRate,$FrameType)
{
//
// open database
echo "Starting Movie";
...
$path_tmp = PATH_TMP;
$list1 = fopen("$path_tmp/zm_list.txt","w") or die('Unable to open file');
...
$video_file=$Mon_Name."-".$date1[0]."_".$date1[1].".avi";
$encoder_param ="/usr/bin/mencoder mf://@".PATH_TMP."/zm_list.txt -mf fps=".$fps." -o ".PATH_TMP."/".$video_file." -ovc lavc -lavcopts vcodec=mpeg4:mbd=1:vbitrate=".$BitRate;
$move_movie = "mv " . PATH_TMP."/$video_file ".PATH_TARGET."/$video_file";

// echo $encoder_param > log.txt;
if(filesize(PATH_TMP."/zm_list.txt") > 0) {
